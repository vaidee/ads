'use strict';

// AWS RDS/Aurora TLS certificates are signed by Amazon's own RDS CA, which
// is NOT in Node's default trust store - without this, pg's
// `ssl: { rejectUnauthorized: true }` fails every connection with
// UNABLE_TO_GET_ISSUER_CERT_LOCALLY. Bundled as a JS string (rather than a
// separate .pem asset) so esbuild inlines it automatically - no extra copy
// step needed in terraform/build.tf.
//
// Region-scoped to ap-southeast-1 (this stack's only region). Regenerate with:
//   curl -s https://truststore.pki.rds.amazonaws.com/ap-southeast-1/ap-southeast-1-bundle.pem
// if the deploy region ever changes, or swap in the global bundle
// (https://truststore.pki.rds.amazonaws.com/global/global-bundle.pem) for a
// region-independent version.
module.exports = `
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`;
