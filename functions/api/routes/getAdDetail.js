...
// publish records + talent detections (SPEC_v2 V2-1 - empty array for ads
// with no client_id/no detections, no special-casing needed).
module.exports = async (event) => {
  const { id } = event.pathParameters;
  const ad = await adsRepo.findById(id);
  if (!ad) return notFound('Ad not found');

  const [flags, comments, statusHistory, publishRecords, talentDetections, playbackUrl] = await Promise.all([
  ...
  return ok({ ad, playbackUrl, flags, comments, statusHistory, publishRecords, talentDetections });
};
